...
import { Newspaper, TrendingUp, PieChart, Video, Play, Calendar, Flame, Search, Share2, SunMoon } from "lucide-react";
...
const Home = () => {
  ...
  const politicsNews = useMemo(() =>
    articles
      .filter(a => a.category === "سياسة" && new Date(a.date) >= startSyncDate)
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
      .slice(0, 4)
  , [articles]);
  const economyNews = useMemo(() =>
    articles
      .filter(a => a.category === "اقتصاد" && new Date(a.date) >= startSyncDate)
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
      .slice(0, 4)
  , [articles]);
  ...
  // شارة جديد
  const isNew = (date: string) => (today.getTime() - new Date(date).getTime()) < 24 * 60 * 60 * 1000;
    ...
    <div className="container mx-auto py-6 px-2">
      ...
      <nav className="flex items-center justify-between mb-4 py-2 px-2 md:px-6 rounded-lg bg-white/80 dark:bg-gray-900/70 shadow">
        <div className="flex items-center gap-4">
          <Link to="/" className="text-2xl font-bold text-news-accent">مصدر بلس</Link>
          <Link to="/news" className="hover:text-news-accent text-gray-600 font-medium">الأخبار</Link>
          <Link to="/politics" className="hover:text-news-accent text-gray-600 font-medium">السياسة</Link>
          <Link to="/economy" className="hover:text-news-accent text-gray-600 font-medium">الاقتصاد</Link>
          <Link to="/videos" className="hover:text-news-accent text-gray-600 font-medium">فيديوهات</Link>
        </div>
        <button
      ...
      <div className="mb-10">
        <div className="flex justify-between items-center mb-4 pb-2 border-b-2 border-news-accent">
          ...
          <Link to="/news" className="text-news-accent hover:underline text-sm font-medium">عرض الكل</Link>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {latestNews.map(article => (
            <div key={article.id} className="bg-white dark:bg-gray-900 rounded-lg overflow-hidden shadow hover:shadow-lg flex flex-col group transition">
              {article.image && (
                <img
                  src={article.image}
                  alt={article.title}
              ...
              <div className="p-4 flex-1 flex flex-col">
                <div className="flex items-center gap-2 mb-2">
                  {isNew(article.date) && (
                    <span className="bg-green-100 text-green-700 text-xs px-2 py-0.5 rounded-full">جديد</span>
                  )}
                  <span className="text-xs text-gray-400">{new Date(article.date).toLocaleDateString('ar-EG')}</span>
                  {article.source && (
                    <span className="text-xs text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full">{article.source}</span>
                  )}
                </div>
                <h3 className="font-bold text-lg mb-1 group-hover:text-news-accent transition">{article.title}</h3>
                <p className="text-gray-600 text-sm mb-3 flex-1 line-clamp-2">{article.excerpt}</p>
                <div className="flex items-center gap-2 mt-auto">
                  <Link to={article.url || "#"} target="_blank" rel="noopener noreferrer" className="text-news-accent text-xs hover:underline">التفاصيل</Link>
                  <button onClick={() => handleShare(article.url || window.location.href)} title="مشاركة الخبر" className="ml-auto text-gray-400 hover:text-news-accent text-xs flex items-center gap-1">
        ...
        <Card className="h-full">
          <CardContent className="p-6">
            <div className="flex justify-between items-center mb-4 pb-2 border-b-2 border-news-accent">
              ...
              <Link to="/politics" className="text-news-accent hover:underline text-sm font-medium">عرض الكل</Link>
            </div>
            <div className="space-y-4">
              {politicsNews.length > 0 ? (
                politicsNews.map(article => (
                  <div key={article.id} className="border-b pb-3 mb-3 last:border-0 last:mb-0 last:pb-0">
                    <Link to={article.url || "#"} className="block group">
                      <h3 className="text-lg font-bold mb-1 group-hover:text-news-accent transition">{article.title}</h3>
                      <p className="text-gray-600 mb-2 text-sm line-clamp-2">{article.excerpt}</p>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-500 text-xs">{new Date(article.date).toLocaleDateString('ar-EG')}</span>
                        {article.source && (
                          <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">{article.source}</span>
                        )}
                      </div>
                    </Link>
                  </div>
        ...
        <Card className="h-full">
          <CardContent className="p-6">
            <div className="flex justify-between items-center mb-4 pb-2 border-b-2 border-news-accent">
              ...
              <Link to="/economy" className="text-news-accent hover:underline text-sm font-medium">عرض الكل</Link>
            </div>
            <div className="space-y-4">
        ...
        <Card className="h-full">
          <CardContent className="p-6">
            <div className="flex justify-between items-center mb-4 pb-2 border-b-2 border-news-accent">
              ...
              <Link to="/videos" className="text-news-accent hover:underline text-sm font-medium">عرض الكل</Link>
            </div>
              ...
              {videoArticles.length > 0 ? (
                videoArticles.map(article => (
                  <div key={article.id} className="relative group">
                    ...
                    <p className="text-xs text-gray-500 mt-1">{new Date(article.date).toLocaleDateString('ar-EG')}</p>
                  </div>
                ))
              ) : (
